...
% Initial conditions [zs, vs, zu, vs]
z0 = [0; 0; 0; 0];

[t, z] = ode45(@(t, z) quarter_car_ode(t, z, ms, mu, ks, bs, kt, bt), tspan, z0);

zs = z(:, 1); vs = z(:, 2); zu = z(:, 3); vu = z(:, 4);

% Compute Road Elevation and Derivative over Time for Plotting
[r_vec, dr_vec] = arrayfun(@road_profile, t);

% Compute Chassis Acceleration: a_s = 1/ms * (-ks*(zs - zu) - bs*(vs - vu))
...
% Extract states
zs = y(1); vs = y(2); zu = y(3); vu = y(4);

% Get road elevation r(t) and road vertical velocity dr/dt
...
% Smoothed Road Profile Function and Analytical Derivative
function [r, dr_dt] = road_profile(t)
% Parameters for a smooth 10 cm step bump around t = 0.2 s
bump_height = 0.10; % 10 cm bump height (m)
t_start = 0.2; % Bump start time (s)
steepness = 50; % Smooth transition factor

r = (bump_height /2) * (1 + tanh(steepness * (t - t_start)))^2;

% Exact analytical derivative: d/dt [tanh(u)] = sech^2(u) * du/dt
